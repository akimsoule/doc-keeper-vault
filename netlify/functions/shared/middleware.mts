 ...
 */
export const safeJsonParse = async (request: Request): Promise<{ success: boolean; data?: any; error?: string }> => {
  try {
...
export const logRequest = async (
  ...
  try {
    ...
    await logService.log({
      action: 'SYSTEM_BACKUP' as any, // Type générique pour les logs API
      entity: 'SYSTEM',
...
export const handleErrors = (fn: Function) => {
  return async (request: Request, netlifyContext: any) => {
    try {
      return await fn(request, netlifyContext);
    } catch (error) {
