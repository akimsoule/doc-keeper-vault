...
interface TagFilterSimpleProps {
...
export const TagFilterSimple: React.FC<TagFilterSimpleProps> = ({
  tags,
  selectedTag,
  setSelectedTag,
  showFilters,
      ...
      <div className="flex flex-wrap gap-1.5 sm:gap-2" role="radiogroup" aria-labelledby="tag-filter-title">
        ...
        {tags.map((tag) => (
          <button
            ...
            style={{
              backgroundColor: selectedTag === tag.name ? (tag.color || getTagColor(tag.name)) : undefined,
              borderColor: selectedTag === tag.name ? (tag.color || getTagColor(tag.name)) : undefined,
            }}
            role="radio"
